			...
			<div class="main-container with-border">
				...
				<ul class="users">
					<c:forEach items="${fans }" var="fan">
						<li>
							<div class="avatar">
								<a href="${ctx }/u/${fan.id }/likes/" target=_blank> <img src="${fan.head }" width="100" height="100" />
								</a>
							</div>
							<div class="user-info">
								<a class="c333" href="${ctx }/u/${fan.id }/likes/" target=_blank> ${fan.name } </a>
								<p class="desc">${fan.bio }</p>
							</div>
						</li>
					</c:forEach>
				</ul>
